...
export function renderAppLayout() {
    ...
    if (!mainApp || mainApp.querySelector('#app-content')) return;

    mainApp.innerHTML = `
        <div id="app-content" class="flex-1 overflow-hidden relative">
            <!-- Tela de Salas (Conversas) -->
            <div id="rooms-screen" class="main-screen flex flex-col h-full absolute inset-0">
                ...
                <div id="conversations-list" class="p-4 flex-1 overflow-y-auto">
                    <div class="text-center py-12 text-gray-500">A carregar conversas...</div>
                </div>
            </div>

            <!-- Tela de Amigos -->
            <div id="friends-screen" class="main-screen flex-col h-full hidden absolute inset-0">
                ...
                <div id="friends-content-container" class="p-4 flex-1 overflow-y-auto">
                    <div id="friends-content-online" class="space-y-3"></div>
                    <div id="friends-content-all" class="space-y-3 hidden"></div>
                    <div id="friends-content-pending" class="space-y-3 hidden"></div>
                </div>
            </div>
        </div>
...
export function injectAllModals() {
    ...
    if (!modalsContainer || modalsContainer.innerHTML !== '') return;
    ...
    modalsContainer.innerHTML = `
        <div id="settings-modal" class="fullscreen-modal hidden fixed inset-0 bg-gray-900 z-[1000] flex flex-col">
            <header class="modal-header flex items-center px-4 py-3">
                 <button data-close-modal="settings-modal" class="text-gray-400 hover:text-white"><i class="fas fa-arrow-left text-xl"></i></button>
                 <h3 class="text-white font-semibold text-lg ml-4">Configurações</h3>
            </header>
            <div class="p-6 flex-1 overflow-y-auto">
                <div class="flex items-center space-x-4 mb-8">
                     <div class="w-16 h-16 rounded-full bg-gray-600 border-2 border-gray-500 overflow-hidden">
                         <img class="user-profile-pic w-full h-full object-cover hidden" alt="Perfil">
                         <div class="user-profile-icon w-full h-full flex items-center justify-center"><i class="fas fa-user text-3xl text-gray-400"></i></div>
                    </div>
                    <div>
                        <h4 class="user-display-name text-white font-medium text-lg">Usuário</h4>
                        <p class="user-display-status text-gray-400 text-sm">Olá! Eu estou a usar o 7Chat.</p>
                    </div>
                </div>
        ...
        <!-- Adicione o HTML de outros modais aqui -->
    `;
}
...
export function updateUserInterface(userData) {
    ...
    document.querySelectorAll('.user-display-status').forEach(el => {
        el.textContent = userData.statusMessage || 'Olá! Eu estou a usar o 7Chat.';
    });
}
